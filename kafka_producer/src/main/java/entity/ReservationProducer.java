...
public class ReservationProducer {
    private String topicName = "reservations";
    ...
    public void produceReservation(Reservation reservation) {
        ...
        String reservationJson = mapper.toJson(reservation, Reservation.class);
        KafkaProducer producer = new KafkaProducer(producerConfig);
        try {
            createTopic();
            Random random = new Random();
            int id = random.nextInt(5 - 1) + 1;
            ProducerRecord<String, String> record = new ProducerRecord<>(topicName, "Sitter" + id, reservationJson);
